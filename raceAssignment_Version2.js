const registration_timeline = {
    ...
    {       
        ...
        // ObjIterator = Object.values(registration_timeline.registers_status);
        ObjIterator = registration_timeline.registers_status;
        ObjIterator.forEach(function(indexKey)
        {
            if((myKey === 'adult runners') && (this.registers(indexKey) === 'Early'))
            {
                racenumber = Math.floor((Math.random()*1000) + 1000);
                console.log(`Early adults run at 9:30am: Race Number is ${racenumber}: Race Number is ${racenumber}`); 
            }
            else if((myKey === 'adult runners') && (this.registers(indexKey) === 'Late'))
            {
